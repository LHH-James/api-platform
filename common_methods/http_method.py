...
def amazon_execute_method_fulfillment_inbound_shipment(request):
    print('/amazon_execute/fulfillmentinboundshipment is recieved a post request')
    data = request.request.body
    # print(data)
    data = data.decode('utf-8').split('&')
    execute_command = {}
    for item in data:
        execute_command[unquote(item.split('=')[0])] = unquote(item.split('=')[1])
    method = eval('interface_fulfillmentinboundshipment.interface_fulfillmentinboundshipment.'+execute_command['method'])
    # 传入的json字符串中有method这个键，通过eval直接寻找对应键值的方法名
    print(execute_command)
    return_data = method(execute_command)
    #处理请求参数
...
def wish_execute_product(request):
    body = request.request.body  # 传入的参数集
    print('data:%s' % body)
    execute_command = {}
    try:
...
def wish_execute_others(request):
    data = request.request.body  # 传入的参数集
    print(data)
    data = data.decode('utf-8').split('&')
    execute_command = {}
    for item in data:
        execute_command[unquote(item.split('=')[0])] = unquote(item.split('=')[1])
    method = eval('interface_wish_others.Others.' + execute_command['method'])  # 传入的json字符串中有method这个键，通过eval直接寻找对应键值的方法名
    print(execute_command)
    return_data = method(execute_command)  # 处理请求参数
    result = return_data
    return result
...
def wish_execute_product_boost(request):
    data = request.request.body  # 传入的参数集
    print(data)
    data = data.decode('utf-8').split('&')
    execute_command = {}
    for item in data:
        execute_command[unquote(item.split('=')[0])] = unquote(item.split('=')[1])
    method = eval('interface_wish_product_boost.Product_boost.' + execute_command['method'])  # 传入的json字符串中有method这个键，通过eval直接寻找对应键值的方法名
    print(execute_command)
    return_data = method(execute_command)  # 处理请求参数
    result = return_data
    return result
